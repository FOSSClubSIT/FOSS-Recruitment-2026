...
class ResponseCollector:
    ...
    def wait_and_collect(self, previous_count: int = 0, previous_last_text: str = "") -> tuple[str, dict]:
        ...
        while True:
            ...
            # Extract real-time status (Thinking / Searching)
            try:
                current_status = self.page.evaluate(f'''() => {{
                    const nodes = document.querySelectorAll("{sel}");
                    if (nodes.length === 0) return "";
                    const lastNode = nodes[nodes.length - 1];
                    const summaries = lastNode.querySelectorAll("summary");
                    if (summaries.length > 0) {{
                        return summaries[summaries.length - 1].innerText.trim();
                    }}
                    return "";
                }}''')
                if current_status and current_status != last_status:
                    print(f"  [Status]: {current_status}", file=sys.stderr)
